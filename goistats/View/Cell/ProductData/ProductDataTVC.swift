...
class ProductDataTVC: UITableViewCell {
    ...
    func configure(with item: [String: String]) {
        let month = safeString(item["month"])
        let quarterly = safeString(item["quarterly"])
        let financialYear = safeString(item["financialyear"]) ?? safeString(item["year"])
        
        let components = [month, quarterly, financialYear]
            .compactMap { $0 }
        
        self.lblPeriod.text = components.joined(separator: " ")
        
        if let indicatorValueStr = item["Indicator1_val"],
